...
export default function ProjectCard({ project }: { project: Project }) {
  return (
    <div className="group relative bg-white border border-surface-border rounded-2xl p-6 hover:border-green-brand/50 hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
      ...
      <div className="absolute top-6 right-6">
        <span className="text-body text-green-brand bg-green-light border border-green-muted px-2 py-0.5 rounded-full">
          {project.category}
        </span>
      </div>
      ...
      <ul className="space-y-1 mb-5">
        {project.highlights.map((h) => (
          <li key={h} className="flex items-start gap-2 text-body text-ink-faint">
            <span className="text-green-brand mt-0.5 flex-shrink-0">▸</span>
            {h}
          </li>
        ))}
      </ul>
